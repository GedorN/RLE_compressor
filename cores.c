...
/*----------------------------------------------------------------------------*/
/** Convers�o RGB -> HSL. Simplesmente segui as f�rmulas.
 *
...
/*----------------------------------------------------------------------------*/
/** Convers�o HSL -> RGB. Simplesmente segui as f�rmulas.
 *
 * Par�metros: Imagem* in: imagem de entrada.
 *             Imagem* out: imagem de sa�da. Deve ter o mesmo tamanho da
 *               imagem de entrada.
...
void HSLParaRGB (Imagem* in, Imagem* out)
{
    ...
    for (row = 0; row < in->altura; row++)
        for (col = 0; col < in->largura; col++)
        {
            ...
            if (s < FLT_EPSILON) // Sem satura��o = sem cor.
            {
...
/*----------------------------------------------------------------------------*/
/** Ajuste simples de cores no espa�o HSL. Consideramos que as imagens j� est�o
 * neste espa�o. N�o use imagens RGB aqui!!!
 *
 * Par�metros: Imagem* in: imagem *HSL* de entrada.
 *             Imagem* out: imagem de sa�da. Deve ter o mesmo tamanho da
 *               imagem de entrada.
 *             float matiz: offset para o matiz.
 *             float saturacao: multiplicador para a satura��o.
 *             float luminancia: constante somada � lumin�ncia.
 *
 * Valor de retorno: nenhum */
